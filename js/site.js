...
(async function(){
  try {
    ...
    if(gallery){ const key=gallery.dataset.gallery; const items=d[key]||[]; gallery.innerHTML=items.length?items.map((x,i)=>`<figure class="tile"><img src="${x.image}" alt="${x.caption||key+' portfolio image '+(i+1)}"><figcaption>${x.caption||''}</figcaption></figure>`).join(''):`<div class="empty-gallery">Add images from the site editor at <b>/admin/</b>.</div>`;}
